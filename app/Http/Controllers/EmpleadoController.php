...
class EmpleadoController extends Controller
{
    ...
    public function ver_per_empleado() {
        ...
        pe.identidad, pe.celular, pe.direccion, pe.id_ciudad_procedencia,  tm.nombre ciudad_procedencia,        
        case when pe.deleted_at is not null then 'Activo' else '' end deleted_at
        from public.per_empleado pe 
        join public.tbl_municipios tm on tm.id = pe.id_ciudad_procedencia        
        where pe.deleted_at is null
        order by 1 desc");
        
        $tbl_municipios_list = DB::select("
        select tm.id, tm.nombre ciudad_procedencia 
        from public.tbl_municipios tm
        where tm.deleted_at is null
        order by 2 asc");        

        return view("configuraciones.empleado")
            ->with("tbl_municipios_list", $tbl_municipios_list)
    ...
    public function guardar_per_empleado(Request $request) {
        ...
        if($accion==1){
            ...
            if( $empleado_existe !=  $empleado_existe_permitido){
                ...
                ", ['telefono'=>$telefono,'domicilio'=>$domicilio,'identidad'=>$identidad,
                    'primer_apellido'=>$primer_apellido,'primer_nombre'=>$primer_nombre,
                    'segundo_apellido'=>$segundo_apellido,'segundo_nombre'=>$segundo_nombre,
                    'correo'=>$correo, 'id_usuario'=>$id_usuario, 'id_ciudad_procedencia'=>$id_ciudad_procedencia,
                    'id_cargo'=>$id_cargo
                ]
                );
        ...
        , ['telefono'=>$telefono,'domicilio'=>$domicilio,'id'=>$id,'identidad'=>$identidad,
            ...
            'segundo_nombre'=>$segundo_nombre, 'correo'=>$correo, 'id_ciudad_procedencia'=>$id_ciudad_procedencia, 'id_cargo'=>$id_cargo
            ]
        );
        $msgSuccess="Registro ".$id." actualizado";
        ...
        $per_empleado_list = DB::select("select * from (
            ...
            from public.per_empleado pe 
            join public.tbl_municipios tm on tm.id = pe.id_ciudad_procedencia
            join public.cat_cargos_empleados cce ON cce.id = pe.id_cargo
            where pe.deleted_at is null
            order by 1 desc
            ) x where id=:id
            ",[
    ...
    public function ver_per_empleado_perfil_usuario( $id_usuario ){
        ...
        $sql_permisos_descripcion = db::select("select id, nombre as permiso from public.seg_permisos
        ...
        COALESCE(TRIM(segundo_apellido||' '),'') ) nombre_medico, u.username
